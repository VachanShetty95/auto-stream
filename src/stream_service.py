...
class StreamService:
    ...
    def __init__(self, config):
        ...
        self.logger = AutoStreamLogger(config=config)
        ...
        game_executables = (
            [config.game_executable] if config.game_executable.strip() else []
        )
        ...
        try:
            ...
            if self.config.game_executable and self.config.game_executable.strip():
                ...
                if self.game_detector.is_game_running(self.config.game_executable):
                    ...
                    self.logger.info(
                        f"Game {self.config.game_executable} not running, using desktop capture"
                    )
            else:
            ...
            self.logger.info(
                f"Starting stream with {'desktop' if use_desktop else 'game-specific'} capture"
            )

            # Start FFmpeg process with fallback for audio device issues
            self.logger.info(
                f"Starting FFmpeg process with command: {' '.join(stream_cmd)}"
            )

            process = subprocess.Popen(
                stream_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
            )
            ...
            if process.poll() is not None:
                ...
                self.logger.log_ffmpeg_output(stdout, stderr, process.returncode)

                # Check if it's an audio device issue
                if (
                    "Could not find audio only device" in stderr
                    or "Error opening input" in stderr
                ):
                    self.logger.warning(
                        "Audio device issue detected. Trying without audio..."
                    )
                    ...
                    for i, arg in enumerate(stream_cmd):
                        if (
                            arg == "-f"
                            and i + 1 < len(stream_cmd)
                            and stream_cmd[i + 1] == "dshow"
                        ):
                    ...
                    if process.poll() is not None:
                        stdout, stderr = process.communicate()
                        self.logger.log_ffmpeg_output(
                            stdout, stderr, process.returncode
                        )
                        print(
                            f"FFmpeg failed to start even without audio. Check logs for details."
                        )
                        return False
                else:
            ...
            self.logger.info(
                f"FFmpeg process started successfully (PID: {process.pid})"
            )
            ...
            # Log stream start
            stream_url = (
                f"{self.config.youtube_stream_url}/{self.config.youtube_stream_key}"
            )
            ...
            if self.discord_notifier:
                self.discord_notifier.notify_stream_started(
                    game_name=game_name, custom_message=self.config.discord_message
                )
    ...
    def stop_stream(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Stopping stream for {game_name} (capture: {capture_type})"
            )
            ...
            if self.discord_notifier:
                self.discord_notifier.notify_stream_stopped(
                    game_name=game_name, duration=duration_str
                )
    ...
    def start_service(self):
        ...
        if not self.config.game_executable.strip():
            self.logger.info(
                "No game executable configured - starting desktop-only streaming"
            )
            ...
            self.logger.info(
                f"Starting game monitoring for: {self.config.game_executable}"
            )
    ...
    def check_stream_health(self) -> bool:
        ...
        if process.poll() is not None:
            ...
            stdout, stderr = process.communicate()
            self.logger.error(
                f"FFmpeg process terminated unexpectedly. Return code: {process.returncode}"
            )
