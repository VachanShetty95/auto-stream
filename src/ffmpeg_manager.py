    ...
    def is_installed(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [str(self.ffmpeg_path), "-version"], capture_output=True, timeout=10
            )
...
"""
    ...
    def generate_stream_command(
        ...
        Args:
            ...
            enable_audio: If True, use real audio input; if False, use silent audio
        """
        ...
        if platform.system() == "Windows":
            ...
            if use_desktop or not game_title:
                ...
                # Always add audio input - real audio if enabled, silent if disabled
                if enable_audio:
                    cmd.extend(
                        ["-f", "dshow", "-i", 'audio="Stereo Mix (Realtek(R) Audio)"']
                    )
                else:
                    # Use silent audio source
                    cmd.extend(
                        [
                            "-f",
                            "lavfi",
            ...
            if display and session_type == "wayland":
                print(
                    f"Detected mixed environment: session={session_type}, display={display}"
                )
                print("Using X11 capture since DISPLAY is available")
                # Use X11 capture
                cmd.extend(
                    ["-f", "x11grab", "-framerate", str(framerate), "-i", display]
                )
                ...
                try:
                    ...
                    if shutil.which("wf-recorder"):
                        ...
                        cmd.extend(
                            [
                                ...
                                "video4linux2",
                                "-i",
                                "/dev/video0",  # This might work if pipewire creates a virtual camera
                            ]
                        )
                    else:
                        # Fallback to test pattern with warning
                        print(
                            "Wayland screen capture not available, using test pattern"
                        )
                        cmd.extend(
                    ...
                    cmd.extend(["-f", "lavfi", "-i", "testsrc2=size=1280x720:rate=30"])
        ...
        # Audio settings (always include audio encoding)
        cmd.extend(["-c:a", "aac", "-b:a", "128k", "-ar", "44100"])

        # Output settings
        cmd.extend(["-f", "flv", f"{stream_url}/{stream_key}"])
    ...
    def test_stream_command(self, cmd: List[str], duration: int = 10) -> bool:
        ...
        try:
            ...
            result = subprocess.run(test_cmd, capture_output=True, timeout=duration + 5)
