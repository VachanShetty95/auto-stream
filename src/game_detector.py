...
class GameDetector:
        ...
        for process in processes:
            ...
            for target_game in self.game_executables:
                if target_game in process_name:
                    ...
                    original_name = next(
                        (exe for exe in self.game_executables if exe.lower() == target_game),
                        target_game
                    )
    ...
    def is_game_running(self, game_executable: str) -> bool:
        ...
        return game_executable.lower() in [game.lower() for game in running_games.keys()]
    ...
    def get_all_game_processes(self) -> List[Dict]:
        ...
        for process in processes:
            ...
            skip_patterns = [
                'system', 'registry', 'winlogon', 'csrss', 'smss', 'lsass',
                'services', 'spoolsv', 'explorer', 'dwm', 'taskhost',
                'chrome', 'firefox', 'notepad', 'cmd', 'powershell',
                'python', 'java', 'node', 'code', 'discord', 'steam',
                'origin', 'uplay', 'epicgames', 'battle.net'
            ]
            
            # Include if it's an .exe and not in skip patterns
            if (name.endswith('.exe') and 
                not any(pattern in name for pattern in skip_patterns) and
                not any(pattern in exe_path for pattern in skip_patterns)):
