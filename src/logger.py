...
class AutoStreamLogger:
    ...
    def __init__(
        self, log_dir: str = "logs", log_level: int = logging.INFO, config=None
    ):
        ...
        Args:
            ...
            config: Optional config object to get log level from
        """
        self.log_dir = Path(log_dir)

        # Use config log level if provided, otherwise use passed log_level
    ...
    def _setup_logger(self) -> None:
        ...
        self.logger.debug(f"Log level set to: {logging.getLevelName(self.log_level)}")
    ...
    def log_ffmpeg_output(self, stdout: str, stderr: str, return_code: int) -> None:
        """Log FFmpeg output with appropriate level based on return code."""
        if return_code == 0:
            if stdout.strip():
                self.debug(f"FFmpeg stdout: {stdout}")
            if stderr.strip():
                self.debug(f"FFmpeg stderr: {stderr}")
        else:
            if stdout.strip():
                self.error(f"FFmpeg stdout: {stdout}")
            if stderr.strip():
                self.error(f"FFmpeg stderr: {stderr}")
            self.error(f"FFmpeg process terminated. Return code: {return_code}")
    ...
    def log_config(self, config) -> None:
        ...
        self.debug(f"  - Stream Quality: {config.stream_quality}")
        self.debug(f"  - Framerate: {config.stream_framerate}")
        self.debug(f"  - Bitrate: {config.stream_bitrate}")
        self.debug(f"  - Check Interval: {config.check_interval}")
        self.debug(f"  - Auto Start: {config.auto_start}")
        self.debug(f"  - Enable Audio: {config.enable_audio}")
        self.debug(f"  - Audio Device: {config.audio_device}")
        self.debug(f"  - Log Level: {logging.getLevelName(config.log_level)}")

        if config.game_executable.strip():
            self.debug(f"  - Game Executable: {config.game_executable}")
        else:
            self.debug("  - Game Executable: None (desktop-only mode)")

        if config.discord_bot_token:
            self.debug("  - Discord: Enabled")
        else:
            self.debug("  - Discord: Disabled")
